...
public class Node {
	...
	public static void insert(Node m, Line a)
	{
		...
		int d1 = ccw(m.line.p1, m.line.p2, a.p1); 
		int d2 = ccw(m.line.p1, m.line.p2, a.p2);

		if ((d1 == 1) && (d2 == 1)) // if both are counter-clockwise, insert to the left
		{
			if (m.leftchild == null)
		...
		else if ((d1 == -1) && (d2 == -1)) // if both are clockwise, insert to the right
		{
			//both are clockwise
...
//	 {
//	 	int x = ccw(new Point(0.0,.6),new Point(1.0,.5),new Point(1.0,.5));
